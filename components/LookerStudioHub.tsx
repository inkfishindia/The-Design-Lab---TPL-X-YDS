import React, { useState, useEffect } from 'react';
import { LookerStudioIcon } from './icons/LookerStudioIcon';
import { Card } from './ui/Card';
import { Input } from './ui/Input';
import { Button } from './ui/Button';

interface LookerStudioHubProps {
  isAuthenticated: boolean;
}

export const LookerStudioHub: React.FC<LookerStudioHubProps> = ({ isAuthenticated }) => {
  const [reportUrl, setReportUrl] = useState('');
  const [inputUrl, setInputUrl] = useState('');

  useEffect(() => {
    try {
      const savedUrl = localStorage.getItem('lookerStudio_reportUrl');
      if (savedUrl) {
        setReportUrl(JSON.parse(savedUrl));
        setInputUrl(JSON.parse(savedUrl));
      }
    } catch (error) {
      console.error("Failed to load Looker Studio URL from localStorage", error);
    }
  }, []);

  useEffect(() => {
    if (reportUrl) {
      localStorage.setItem('lookerStudio_reportUrl', JSON.stringify(reportUrl));
    } else {
      localStorage.removeItem('lookerStudio_reportUrl');
    }
  }, [reportUrl]);

  const handleLoadReport = (e: React.FormEvent) => {
    e.preventDefault();
    const trimmedUrl = inputUrl.trim();
    if (trimmedUrl.startsWith('https://lookerstudio.google.com/embed/')) {
      try {
        const url = new URL(trimmedUrl);
        url.searchParams.set('rm', 'minimal');
        setReportUrl(url.toString());
      } catch (error) {
        alert('Invalid URL format. Please check the URL and try again.');
      }
    } else {
      alert('Please enter a valid Looker Studio embed URL. It should start with "https://lookerstudio.google.com/embed/".');
    }
  };

  if (!isAuthenticated) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center p-4">
        <LookerStudioIcon className="w-12 h-12 mx-auto text-text-muted/50" />
        <p className="mt-2 text-sm text-text-muted">Please sign in to embed and view your reports.</p>
      </div>
    );
  }

  return (
    <div className="flex flex-col h-full gap-4">
        <div>
            <form onSubmit={handleLoadReport} className="flex flex-col sm:flex-row gap-3">
                <div className="flex-grow">
                    <Input
                        value={inputUrl}
                        onChange={e => setInputUrl(e.target.value)}
                        placeholder="Paste Looker Studio Embed URL..."
                        aria-label="Looker Studio Embed URL"
                    />
                </div>
                <Button type="submit">
                  Load Report
                </Button>
            </form>
            <p className="text-xs text-text-muted mt-2">
                To get the URL: In Looker Studio, go to your report &gt; Share &gt; Embed report &gt; Copy the 'Embed URL'.
            </p>
        </div>

        <div className="flex-grow min-h-0">
          {reportUrl ? (
            <div className="w-full h-full overflow-hidden border border-dark-border rounded-lg shadow-inner">
              <iframe
                key={reportUrl}
                width="100%"
                height="100%"
                src={reportUrl}
                frameBorder="0"
                style={{ border: 0 }}
                allowFullScreen
                sandbox="allow-storage-access-by-user-activation allow-scripts allow-same-origin allow-forms allow-popups"
                title="Looker Studio Report"
              ></iframe>
            </div>
          ) : (
            <div className="h-full flex items-center justify-center text-center p-4 border-2 border-dashed border-dark-border rounded-lg">
              <div>
                <LookerStudioIcon className="w-10 h-10 mx-auto text-text-muted/40" />
                <p className="mt-4 text-text-muted">Your embedded report will be displayed here.</p>
              </div>
            </div>
          )}
        </div>
    </div>
  );
};

  ...
  return (
    <div>
      <div className="text-center">
        <h2 className="text-3xl font-display font-bold text-cream">Competitor Hub</h2>
        <p className="mt-2 text-lg text-cream/70">Add competitors to automatically track their web presence.</p>
      </div>
      ...
      <div className="mt-8 max-w-3xl mx-auto">
        <Card>
          <form onSubmit={handleAddCompetitor} className="flex gap-3">
            ...
            <Button type="submit" disabled={isApiKeyMissing || !newCompetitor.trim()}>
              Add
            </Button>
          </form>
          <ApiKeyWarning />
        ...
        <div className="mt-6 space-y-3">
          {competitors.length > 0 && (
            <div className="flex justify-end items-center mb-4">
              {saveError && <p className="text-xs text-error-red mr-4">{saveError}</p>}
                {isAuthenticated ? (
                  <Button
                  ...
                  >
                    {isSaving ? 'Saving...' : (saveSuccess ? 'Saved!' : 'Save List to Drive')}
                  </Button>
                ) : (
                  <p className="text-xs text-cream/60">Sign in to save to Google Drive.</p>
                )}
            </div>
          )}
          {competitors.map((comp, i) => (
            <Card key={i}>
              <div className="flex justify-between items-center mb-3">
                <h4 className="font-semibold text-midnight-navy text-base">{comp.name}</h4>
                <Button onClick={() => handleDeleteCompetitor(comp.name)} variant="danger" size="sm">Remove</Button>
              </div>
              
              {comp.isLoading && <p className="text-sm text-midnight-navy/70">Finding profiles...</p>}
              {comp.error && <p className="text-sm text-error-red">{comp.error}</p>}
              
              {comp.socials && (
                <div className="flex flex-wrap gap-x-4 gap-y-1 pt-3 border-t border-midnight-navy/10">
                  {comp.socials.website ? <a href={comp.socials.website} target="_blank" rel="noopener noreferrer" className="text-sm text-heritage-blue hover:underline">Website</a> : <span className="text-sm text-midnight-navy/50">Website: N/A</span>}
                  {comp.socials.twitter ? <a href={comp.socials.twitter} target="_blank" rel="noopener noreferrer" className="text-sm text-heritage-blue hover:underline">Twitter/X</a> : <span className="text-sm text-midnight-navy/50">Twitter/X: N/A</span>}
                  {comp.socials.linkedin ? <a href={comp.socials.linkedin} target="_blank" rel="noopener noreferrer" className="text-sm text-heritage-blue hover:underline">LinkedIn</a> : <span className="text-sm text-midnight-navy/50">LinkedIn: N/A</span>}
                </div>
              )}
            </Card>
          ))}
          {competitors.length === 0 && (
            <div className="text-center py-10">
              <p className="text-cream/60">Your competitor list is empty.</p>
            </div>
          )}
        </div>
      </div>

...
import type { GoogleUser, TokenResponse, NavItem, ViewName } from './types';
...
import { SystemMapIcon } from './components/icons/SystemMapIcon';

import { BusinessView } from './components/BusinessView';
import { MarketingView } from './components/MarketingView';
import { CustomersView } from './components/CustomersView';
import { ReportsView } from './components/ReportsView';
import { VisualSystemMapView } from './components/VisualSystemMapView';
...
const allNavItems: NavItem[] = [
    ...
    { viewName: 'business', label: 'Business Strategy', icon: <BrainIcon className="w-6 h-6" /> },
    { viewName: 'visualSystemMapView', label: 'System Map', icon: <SystemMapIcon className="w-6 h-6" />},
...
const AppContent: React.FC = () => {
    ...
    const [dashboardGreeting, setDashboardGreeting] = useState('Dashboard');
    ...
    const getGreeting = useCallback(() => {
        ...
        const firstName = user?.name.split(' ')[0] || 'Dan';

        let personalGreetingTemplates: string[]; 

        if (hour < 5) { // Late Night/Early Morning
            personalGreetingTemplates = [`Burning the midnight oil, ${firstName}?`, `Late night session, ${firstName}?`, `Still at it, ${firstName}?`];
        } else if (hour < 12) { // Morning
            personalGreetingTemplates = [
                `Good morning, ${firstName}. Ready to create?`,
                `Let's design your day, ${firstName}.`,
                `What's the big idea today, ${firstName}?`
            ];
        } else if (hour < 17) { // Afternoon
            personalGreetingTemplates = [
                `Hey ${firstName}, how's the day shaping up?`,
                `Keep the momentum going, ${firstName}.`,
                `Here's to a productive afternoon, ${firstName}.`
            ];
        } else { // Evening
            personalGreetingTemplates = [
                `Good evening, ${firstName}.`,
                `${firstName}, let's wrap up the day strong.`,
                `Time for some strategic thinking, ${firstName}?`
            ];
        }

        return personalGreetingTemplates[Math.floor(Math.random() * personalGreetingTemplates.length)];

    }, [user]);

    useEffect(() => {
        setDashboardGreeting(getGreeting()); // Set initial greeting
    ...
    const views = useMemo(() => ({
        ...
        visualSystemMapView: <VisualSystemMapView isAuthenticated={isAuthenticated} token={token} />,
    ...
    return (
        <div className="flex min-h-screen font-sans bg-dark-bg text-text-light">
            <Sidebar 
            ...
            <div className="flex-grow flex flex-col w-full">
                {!isSettingsView && (
                    <header className="flex-shrink-0 flex items-center justify-between h-[72px] border-b border-dark-border px-6 sticky top-0 bg-dark-bg z-30">
                        <h1 className="text-2xl font-bold font-display text-text-light">
                            {activeView === 'dashboard'
                                ? dashboardGreeting
                                : allNavItems.find(item => item.viewName === activeView)?.label}
                        </h1>
